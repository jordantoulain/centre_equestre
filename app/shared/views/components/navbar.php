<nav class="flex p-4 w-screen justify-between">
    <button class="btn-icon-outline sm:hidden" type="button" onclick="document.dispatchEvent(new CustomEvent('basecoat:sidebar'))">
        <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24"><path fill="#ffffff" fill-rule="evenodd" d="M3.5 5a1 1 0 0 0 0 2h17a1 1 0 1 0 0-2h-17Zm-1 7a1 1 0 0 1 1-1h17a1 1 0 1 1 0 2h-17a1 1 0 0 1-1-1Zm0 6.001a1 1 0 0 1 1-1h17a1 1 0 1 1 0 2h-17a1 1 0 0 1-1-1Z" clip-rule="evenodd"/></svg>
        <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24"><path fill="#000000" fill-rule="evenodd" d="M3.5 5a1 1 0 0 0 0 2h17a1 1 0 1 0 0-2h-17Zm-1 7a1 1 0 0 1 1-1h17a1 1 0 1 1 0 2h-17a1 1 0 0 1-1-1Zm0 6.001a1 1 0 0 1 1-1h17a1 1 0 1 1 0 2h-17a1 1 0 0 1-1-1Z" clip-rule="evenodd"/></svg>
    </button>
    <div class="gap-5 hidden sm:flex">
        <a href="./?p=home" class="btn-ghost size-12 w-auto">
            <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#ffffff">
                <g clip-path="url(#clip0_4418_8583)">
                <path d="M20.86 8.36985L13.93 2.82985C12.86 1.96985 11.13 1.96985 10.07 2.81985L3.14002 8.36985C2.36002 8.98985 1.86002 10.2998 2.03002 11.2798L3.36002 19.2398C3.60002 20.6598 4.96002 21.8098 6.40002 21.8098H17.6C19.03 21.8098 20.4 20.6498 20.64 19.2398L21.97 11.2798C22.13 10.2998 21.63 8.98985 20.86 8.36985ZM12 15.4998C10.62 15.4998 9.50002 14.3798 9.50002 12.9998C9.50002 11.6198 10.62 10.4998 12 10.4998C13.38 10.4998 14.5 11.6198 14.5 12.9998C14.5 14.3798 13.38 15.4998 12 15.4998Z" fill="white" style="fill: var(--fillg);"/>
                </g>
                <defs>
                <clipPath id="clip0_4418_8583">
                <rect width="24" height="24" fill="white"/>
                </clipPath>
                </defs>
            </svg>
            <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#000000">
                <g clip-path="url(#clip0_4418_8583)">
                <path d="M20.86 8.36985L13.93 2.82985C12.86 1.96985 11.13 1.96985 10.07 2.81985L3.14002 8.36985C2.36002 8.98985 1.86002 10.2998 2.03002 11.2798L3.36002 19.2398C3.60002 20.6598 4.96002 21.8098 6.40002 21.8098H17.6C19.03 21.8098 20.4 20.6498 20.64 19.2398L21.97 11.2798C22.13 10.2998 21.63 8.98985 20.86 8.36985ZM12 15.4998C10.62 15.4998 9.50002 14.3798 9.50002 12.9998C9.50002 11.6198 10.62 10.4998 12 10.4998C13.38 10.4998 14.5 11.6198 14.5 12.9998C14.5 14.3798 13.38 15.4998 12 15.4998Z" fill="white" style="fill: var(--fillg);"/>
                </g>
                <defs>
                <clipPath id="clip0_4418_8583">
                <rect width="24" height="24" fill="white"/>
                </clipPath>
                </defs>
            </svg>
            <span>Accueil</span>
        </a>
        <a href="#" class="btn-ghost size-12 w-auto">
            <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#ffffff">
                <g clip-path="url(#clip0_4418_8478)">
                <path d="M14.9301 2.5V8.4C14.9301 8.84 14.4101 9.06 14.0901 8.77L12.3401 7.16C12.1501 6.98 11.8501 6.98 11.6601 7.16L9.91007 8.76C9.59007 9.06 9.07007 8.83 9.07007 8.4V2.5C9.07007 2.22 9.29007 2 9.57007 2H14.4301C14.7101 2 14.9301 2.22 14.9301 2.5Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M16.98 2.06086C16.69 2.02086 16.43 2.27086 16.43 2.56086V8.58086C16.43 9.34086 15.98 10.0309 15.28 10.3409C14.58 10.6409 13.77 10.5109 13.21 9.99086L12.34 9.19086C12.15 9.01086 11.86 9.01086 11.66 9.19086L10.79 9.99086C10.43 10.3309 9.96 10.5009 9.49 10.5009C9.23 10.5009 8.97 10.4509 8.72 10.3409C8.02 10.0309 7.57 9.34086 7.57 8.58086V2.56086C7.57 2.27086 7.31 2.02086 7.02 2.06086C4.22 2.41086 3 4.30086 3 7.00086V17.0009C3 20.0009 4.5 22.0009 8 22.0009H16C19.5 22.0009 21 20.0009 21 17.0009V7.00086C21 4.30086 19.78 2.41086 16.98 2.06086ZM17.5 18.7509H9C8.59 18.7509 8.25 18.4109 8.25 18.0009C8.25 17.5909 8.59 17.2509 9 17.2509H17.5C17.91 17.2509 18.25 17.5909 18.25 18.0009C18.25 18.4109 17.91 18.7509 17.5 18.7509ZM17.5 14.7509H13.25C12.84 14.7509 12.5 14.4109 12.5 14.0009C12.5 13.5909 12.84 13.2509 13.25 13.2509H17.5C17.91 13.2509 18.25 13.5909 18.25 14.0009C18.25 14.4109 17.91 14.7509 17.5 14.7509Z" fill="white" style="fill: var(--fillg);"/>
                </g>
                <defs>
                <clipPath id="clip0_4418_8478">
                <rect width="24" height="24" fill="white"/>
                </clipPath>
                </defs>
            </svg>
            <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#000000">
                <g clip-path="url(#clip0_4418_8478)">
                <path d="M14.9301 2.5V8.4C14.9301 8.84 14.4101 9.06 14.0901 8.77L12.3401 7.16C12.1501 6.98 11.8501 6.98 11.6601 7.16L9.91007 8.76C9.59007 9.06 9.07007 8.83 9.07007 8.4V2.5C9.07007 2.22 9.29007 2 9.57007 2H14.4301C14.7101 2 14.9301 2.22 14.9301 2.5Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M16.98 2.06086C16.69 2.02086 16.43 2.27086 16.43 2.56086V8.58086C16.43 9.34086 15.98 10.0309 15.28 10.3409C14.58 10.6409 13.77 10.5109 13.21 9.99086L12.34 9.19086C12.15 9.01086 11.86 9.01086 11.66 9.19086L10.79 9.99086C10.43 10.3309 9.96 10.5009 9.49 10.5009C9.23 10.5009 8.97 10.4509 8.72 10.3409C8.02 10.0309 7.57 9.34086 7.57 8.58086V2.56086C7.57 2.27086 7.31 2.02086 7.02 2.06086C4.22 2.41086 3 4.30086 3 7.00086V17.0009C3 20.0009 4.5 22.0009 8 22.0009H16C19.5 22.0009 21 20.0009 21 17.0009V7.00086C21 4.30086 19.78 2.41086 16.98 2.06086ZM17.5 18.7509H9C8.59 18.7509 8.25 18.4109 8.25 18.0009C8.25 17.5909 8.59 17.2509 9 17.2509H17.5C17.91 17.2509 18.25 17.5909 18.25 18.0009C18.25 18.4109 17.91 18.7509 17.5 18.7509ZM17.5 14.7509H13.25C12.84 14.7509 12.5 14.4109 12.5 14.0009C12.5 13.5909 12.84 13.2509 13.25 13.2509H17.5C17.91 13.2509 18.25 13.5909 18.25 14.0009C18.25 14.4109 17.91 14.7509 17.5 14.7509Z" fill="white" style="fill: var(--fillg);"/>
                </g>
                <defs>
                <clipPath id="clip0_4418_8478">
                <rect width="24" height="24" fill="white"/>
                </clipPath>
                </defs>
            </svg>
            <span>Présentation</span>
        </a>
        <a href="#" class="btn-ghost size-12 w-auto">
            <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#fff">
                <g clip-path="url(#clip0_655_9407)">
                <path d="M17.5599 12C17.5599 12.4059 17.5118 12.7981 17.4328 13.1766C17.4042 13.3137 17.4687 13.4544 17.594 13.5171L21.0965 15.2683C21.2577 15.3489 21.4534 15.2703 21.5084 15.0987C21.8215 14.1226 21.9999 13.0897 21.9999 12V11.94C21.9999 11.7743 21.8656 11.64 21.6999 11.64H17.8599C17.6942 11.64 17.5599 11.7743 17.5599 11.94V12Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M6.445 12.005C6.4418 12.0018 6.44 11.9975 6.44 11.9929V11.8946C6.44 11.754 6.32603 11.64 6.18544 11.64H2.3C2.13431 11.64 2 11.7743 2 11.94V12C2 13.3901 2.28816 14.704 2.79426 15.9025C2.86394 16.0676 3.06421 16.1286 3.21737 16.0356L6.56313 14.0055C6.68386 13.9323 6.73637 13.785 6.69567 13.6498C6.54128 13.1367 6.45075 12.5882 6.45 12.0171C6.45 12.0126 6.4482 12.0082 6.445 12.005Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M6.14 7.19H2.3C2.13431 7.19 2 7.32432 2 7.49V9.83C2 9.99569 2.13431 10.13 2.3 10.13H6.14C6.30569 10.13 6.44 9.99569 6.44 9.83V7.49C6.44 7.32432 6.30569 7.19 6.14 7.19Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M21.6996 7.19H17.8596C17.6939 7.19 17.5596 7.32432 17.5596 7.49V9.83C17.5596 9.99569 17.6939 10.13 17.8596 10.13H21.6996C21.8653 10.13 21.9996 9.99569 21.9996 9.83V7.49C21.9996 7.32432 21.8653 7.19 21.6996 7.19Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M21.6996 2H17.8596C17.6939 2 17.5596 2.13431 17.5596 2.3V5.39C17.5596 5.55569 17.6939 5.69 17.8596 5.69H21.6996C21.8653 5.69 21.9996 5.55569 21.9996 5.39V2.3C21.9996 2.13431 21.8653 2 21.6996 2Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M17.0658 14.928C16.9228 14.8563 16.7494 14.9101 16.6629 15.0447C16.4235 15.4176 16.1367 15.7556 15.8129 16.0589C15.7003 16.1643 15.6775 16.3364 15.7655 16.463L17.9696 19.6303C18.0679 19.7714 18.2647 19.8016 18.397 19.6919C19.2888 18.9525 20.0474 18.0648 20.6299 17.0595C20.7147 16.9132 20.6556 16.7274 20.5045 16.6517L17.0658 14.928Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M10.9295 21.9321C11.1026 21.9501 11.2502 21.8121 11.2502 21.6382V17.7654C11.2502 17.6139 11.1369 17.487 10.9876 17.4611C10.6124 17.3959 10.2512 17.2917 9.90727 17.1546C9.75678 17.0946 9.58272 17.1552 9.51042 17.3001L7.7969 20.7358C7.72185 20.8863 7.78495 21.0695 7.939 21.1369C8.86964 21.5442 9.87417 21.8227 10.9295 21.9321Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M8.26627 16.4393C8.32749 16.3162 8.29664 16.1679 8.19614 16.0742C7.98618 15.8784 7.79248 15.6669 7.61749 15.4399C7.52017 15.3137 7.3437 15.2748 7.20748 15.3577L3.91769 17.3575C3.7719 17.4461 3.72893 17.6386 3.82782 17.7776C4.4526 18.656 5.21273 19.4339 6.07952 20.071C6.22615 20.1788 6.43266 20.1247 6.51372 19.9618L8.26627 16.4393Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M14.4934 17.2562C14.4104 17.1369 14.2542 17.0951 14.1194 17.1495C13.7651 17.2927 13.3956 17.3979 13.0075 17.4686C12.8604 17.4953 12.75 17.6212 12.75 17.7708V21.6473C12.75 21.8216 12.8981 21.9596 13.0714 21.9416C14.3449 21.8093 15.5423 21.4475 16.6203 20.887C16.7771 20.8055 16.8228 20.6046 16.7218 20.4594L14.4934 17.2562Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M6.14 2H2.3C2.13431 2 2 2.13431 2 2.3V5.39C2 5.55569 2.13431 5.69 2.3 5.69H6.14C6.30569 5.69 6.44 5.55569 6.44 5.39V2.3C6.44 2.13431 6.30569 2 6.14 2Z" fill="white" style="fill: var(--fillg);"/>
                </g>
                <defs>
                <clipPath id="clip0_655_9407">
                <rect width="24" height="24" fill="white"/>
                </clipPath>
                </defs>
            </svg>
            <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#000000">
                <g clip-path="url(#clip0_655_9407)">
                <path d="M17.5599 12C17.5599 12.4059 17.5118 12.7981 17.4328 13.1766C17.4042 13.3137 17.4687 13.4544 17.594 13.5171L21.0965 15.2683C21.2577 15.3489 21.4534 15.2703 21.5084 15.0987C21.8215 14.1226 21.9999 13.0897 21.9999 12V11.94C21.9999 11.7743 21.8656 11.64 21.6999 11.64H17.8599C17.6942 11.64 17.5599 11.7743 17.5599 11.94V12Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M6.445 12.005C6.4418 12.0018 6.44 11.9975 6.44 11.9929V11.8946C6.44 11.754 6.32603 11.64 6.18544 11.64H2.3C2.13431 11.64 2 11.7743 2 11.94V12C2 13.3901 2.28816 14.704 2.79426 15.9025C2.86394 16.0676 3.06421 16.1286 3.21737 16.0356L6.56313 14.0055C6.68386 13.9323 6.73637 13.785 6.69567 13.6498C6.54128 13.1367 6.45075 12.5882 6.45 12.0171C6.45 12.0126 6.4482 12.0082 6.445 12.005Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M6.14 7.19H2.3C2.13431 7.19 2 7.32432 2 7.49V9.83C2 9.99569 2.13431 10.13 2.3 10.13H6.14C6.30569 10.13 6.44 9.99569 6.44 9.83V7.49C6.44 7.32432 6.30569 7.19 6.14 7.19Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M21.6996 7.19H17.8596C17.6939 7.19 17.5596 7.32432 17.5596 7.49V9.83C17.5596 9.99569 17.6939 10.13 17.8596 10.13H21.6996C21.8653 10.13 21.9996 9.99569 21.9996 9.83V7.49C21.9996 7.32432 21.8653 7.19 21.6996 7.19Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M21.6996 2H17.8596C17.6939 2 17.5596 2.13431 17.5596 2.3V5.39C17.5596 5.55569 17.6939 5.69 17.8596 5.69H21.6996C21.8653 5.69 21.9996 5.55569 21.9996 5.39V2.3C21.9996 2.13431 21.8653 2 21.6996 2Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M17.0658 14.928C16.9228 14.8563 16.7494 14.9101 16.6629 15.0447C16.4235 15.4176 16.1367 15.7556 15.8129 16.0589C15.7003 16.1643 15.6775 16.3364 15.7655 16.463L17.9696 19.6303C18.0679 19.7714 18.2647 19.8016 18.397 19.6919C19.2888 18.9525 20.0474 18.0648 20.6299 17.0595C20.7147 16.9132 20.6556 16.7274 20.5045 16.6517L17.0658 14.928Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M10.9295 21.9321C11.1026 21.9501 11.2502 21.8121 11.2502 21.6382V17.7654C11.2502 17.6139 11.1369 17.487 10.9876 17.4611C10.6124 17.3959 10.2512 17.2917 9.90727 17.1546C9.75678 17.0946 9.58272 17.1552 9.51042 17.3001L7.7969 20.7358C7.72185 20.8863 7.78495 21.0695 7.939 21.1369C8.86964 21.5442 9.87417 21.8227 10.9295 21.9321Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M8.26627 16.4393C8.32749 16.3162 8.29664 16.1679 8.19614 16.0742C7.98618 15.8784 7.79248 15.6669 7.61749 15.4399C7.52017 15.3137 7.3437 15.2748 7.20748 15.3577L3.91769 17.3575C3.7719 17.4461 3.72893 17.6386 3.82782 17.7776C4.4526 18.656 5.21273 19.4339 6.07952 20.071C6.22615 20.1788 6.43266 20.1247 6.51372 19.9618L8.26627 16.4393Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M14.4934 17.2562C14.4104 17.1369 14.2542 17.0951 14.1194 17.1495C13.7651 17.2927 13.3956 17.3979 13.0075 17.4686C12.8604 17.4953 12.75 17.6212 12.75 17.7708V21.6473C12.75 21.8216 12.8981 21.9596 13.0714 21.9416C14.3449 21.8093 15.5423 21.4475 16.6203 20.887C16.7771 20.8055 16.8228 20.6046 16.7218 20.4594L14.4934 17.2562Z" fill="white" style="fill: var(--fillg);"/>
                <path d="M6.14 2H2.3C2.13431 2 2 2.13431 2 2.3V5.39C2 5.55569 2.13431 5.69 2.3 5.69H6.14C6.30569 5.69 6.44 5.55569 6.44 5.39V2.3C6.44 2.13431 6.30569 2 6.14 2Z" fill="white" style="fill: var(--fillg);"/>
                </g>
                <defs>
                <clipPath id="clip0_655_9407">
                <rect width="24" height="24" fill="white"/>
                </clipPath>
                </defs>
            </svg>
            <span>Montures</span>
        </a>
    </div>
    <div class="gap-2 flex">
        <button
            type="button"
            data-tooltip="Mode sombre/clair"
            data-side="bottom"
            onclick="document.dispatchEvent(new CustomEvent('basecoat:theme'))"
            class="btn-icon-outline size-12"
            >
            <span class="hidden dark:block"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4" /><path d="M12 2v2" /><path d="M12 20v2" /><path d="m4.93 4.93 1.41 1.41" /><path d="m17.66 17.66 1.41 1.41" /><path d="M2 12h2" /><path d="M20 12h2" /><path d="m6.34 17.66-1.41 1.41" /><path d="m19.07 4.93-1.41 1.41" /></svg></span>
            <span class="block dark:hidden"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z" /></svg></span>
        </button>
        <?php if (isset($_SESSION['username'])) { ?>
            <a href="./?p=logout" class="group btn-outline h-12 justify-start pr-4 md:max-w-50">
                <img alt="<?= e($_SESSION['lastname']) ?>" src="https://eu.ui-avatars.com/api/?name=<?= e($_SESSION['lastname']) ?>&size=250" class="size-8 shrink-0 rounded-full">
                <div class="grid flex-1 text-left text-sm leading-tight">
                    <span class="truncate font-medium capitalize"><?= e($_SESSION['firstname']) ?> <?= e($_SESSION['lastname']) ?></span>
                    <span data-tooltip="<?= e($_SESSION['username']) ?>" data-tooltip-after="Se déconnecter" class="text-muted-foreground after:content-[attr(data-tooltip)] truncate text-xs group-hover:after:content-[attr(data-tooltip-after)]"></span>
                </div>
                <svg class="hidden hover:dark:block" xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24"><path fill="#000000" d="M3 11h9.586l-3.5-3.5L10.5 6.086L16.414 12L10.5 17.914L9.086 16.5l3.5-3.5H3v-2Zm11 8.5h5v-15h-5v-2h7v19h-7v-2Z"/></svg>
                <svg class="hidden hover:block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24"><path fill="#ffffff" d="M3 11h9.586l-3.5-3.5L10.5 6.086L16.414 12L10.5 17.914L9.086 16.5l3.5-3.5H3v-2Zm11 8.5h5v-15h-5v-2h7v19h-7v-2Z"/></svg>
            </a>
        <?php }else{ ?>
            <a href="./?p=login" class="btn-primary size-12 w-auto px-5">
                <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24"><path fill="#000000" d="M3 11h9.586l-3.5-3.5L10.5 6.086L16.414 12L10.5 17.914L9.086 16.5l3.5-3.5H3v-2Zm11 8.5h5v-15h-5v-2h7v19h-7v-2Z"/></svg>
                <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24"><path fill="#ffffff" d="M3 11h9.586l-3.5-3.5L10.5 6.086L16.414 12L10.5 17.914L9.086 16.5l3.5-3.5H3v-2Zm11 8.5h5v-15h-5v-2h7v19h-7v-2Z"/></svg>
                <span>Connexion</span>
            </a>
        <?php } ?>
    </div>
</nav>

<aside class="sidebar sm:hidden" data-side="left" aria-hidden="true">
  <nav aria-label="Sidebar navigation">
    <section class="scrollbar">
      <div role="group" aria-labelledby="group-label-content-1">
        <h3 id="group-label-content-1">Navigation</h3>

        <ul>

        <li>
            <a href="./?p=home">
                <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#ffffff">
                    <g clip-path="url(#clip0_4418_8583)">
                    <path d="M20.86 8.36985L13.93 2.82985C12.86 1.96985 11.13 1.96985 10.07 2.81985L3.14002 8.36985C2.36002 8.98985 1.86002 10.2998 2.03002 11.2798L3.36002 19.2398C3.60002 20.6598 4.96002 21.8098 6.40002 21.8098H17.6C19.03 21.8098 20.4 20.6498 20.64 19.2398L21.97 11.2798C22.13 10.2998 21.63 8.98985 20.86 8.36985ZM12 15.4998C10.62 15.4998 9.50002 14.3798 9.50002 12.9998C9.50002 11.6198 10.62 10.4998 12 10.4998C13.38 10.4998 14.5 11.6198 14.5 12.9998C14.5 14.3798 13.38 15.4998 12 15.4998Z" fill="white" style="fill: var(--fillg);"/>
                    </g>
                    <defs>
                    <clipPath id="clip0_4418_8583">
                    <rect width="24" height="24" fill="white"/>
                    </clipPath>
                    </defs>
                </svg>
                <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#000000">
                    <g clip-path="url(#clip0_4418_8583)">
                    <path d="M20.86 8.36985L13.93 2.82985C12.86 1.96985 11.13 1.96985 10.07 2.81985L3.14002 8.36985C2.36002 8.98985 1.86002 10.2998 2.03002 11.2798L3.36002 19.2398C3.60002 20.6598 4.96002 21.8098 6.40002 21.8098H17.6C19.03 21.8098 20.4 20.6498 20.64 19.2398L21.97 11.2798C22.13 10.2998 21.63 8.98985 20.86 8.36985ZM12 15.4998C10.62 15.4998 9.50002 14.3798 9.50002 12.9998C9.50002 11.6198 10.62 10.4998 12 10.4998C13.38 10.4998 14.5 11.6198 14.5 12.9998C14.5 14.3798 13.38 15.4998 12 15.4998Z" fill="white" style="fill: var(--fillg);"/>
                    </g>
                    <defs>
                    <clipPath id="clip0_4418_8583">
                    <rect width="24" height="24" fill="white"/>
                    </clipPath>
                    </defs>
                </svg>
                <span>Accueil</span>
            </a>
          </li>

          <li>
            <a href="#">
                <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#ffffff">
                    <g clip-path="url(#clip0_4418_8478)">
                    <path d="M14.9301 2.5V8.4C14.9301 8.84 14.4101 9.06 14.0901 8.77L12.3401 7.16C12.1501 6.98 11.8501 6.98 11.6601 7.16L9.91007 8.76C9.59007 9.06 9.07007 8.83 9.07007 8.4V2.5C9.07007 2.22 9.29007 2 9.57007 2H14.4301C14.7101 2 14.9301 2.22 14.9301 2.5Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M16.98 2.06086C16.69 2.02086 16.43 2.27086 16.43 2.56086V8.58086C16.43 9.34086 15.98 10.0309 15.28 10.3409C14.58 10.6409 13.77 10.5109 13.21 9.99086L12.34 9.19086C12.15 9.01086 11.86 9.01086 11.66 9.19086L10.79 9.99086C10.43 10.3309 9.96 10.5009 9.49 10.5009C9.23 10.5009 8.97 10.4509 8.72 10.3409C8.02 10.0309 7.57 9.34086 7.57 8.58086V2.56086C7.57 2.27086 7.31 2.02086 7.02 2.06086C4.22 2.41086 3 4.30086 3 7.00086V17.0009C3 20.0009 4.5 22.0009 8 22.0009H16C19.5 22.0009 21 20.0009 21 17.0009V7.00086C21 4.30086 19.78 2.41086 16.98 2.06086ZM17.5 18.7509H9C8.59 18.7509 8.25 18.4109 8.25 18.0009C8.25 17.5909 8.59 17.2509 9 17.2509H17.5C17.91 17.2509 18.25 17.5909 18.25 18.0009C18.25 18.4109 17.91 18.7509 17.5 18.7509ZM17.5 14.7509H13.25C12.84 14.7509 12.5 14.4109 12.5 14.0009C12.5 13.5909 12.84 13.2509 13.25 13.2509H17.5C17.91 13.2509 18.25 13.5909 18.25 14.0009C18.25 14.4109 17.91 14.7509 17.5 14.7509Z" fill="white" style="fill: var(--fillg);"/>
                    </g>
                    <defs>
                    <clipPath id="clip0_4418_8478">
                    <rect width="24" height="24" fill="white"/>
                    </clipPath>
                    </defs>
                </svg>
                <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#000000">
                    <g clip-path="url(#clip0_4418_8478)">
                    <path d="M14.9301 2.5V8.4C14.9301 8.84 14.4101 9.06 14.0901 8.77L12.3401 7.16C12.1501 6.98 11.8501 6.98 11.6601 7.16L9.91007 8.76C9.59007 9.06 9.07007 8.83 9.07007 8.4V2.5C9.07007 2.22 9.29007 2 9.57007 2H14.4301C14.7101 2 14.9301 2.22 14.9301 2.5Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M16.98 2.06086C16.69 2.02086 16.43 2.27086 16.43 2.56086V8.58086C16.43 9.34086 15.98 10.0309 15.28 10.3409C14.58 10.6409 13.77 10.5109 13.21 9.99086L12.34 9.19086C12.15 9.01086 11.86 9.01086 11.66 9.19086L10.79 9.99086C10.43 10.3309 9.96 10.5009 9.49 10.5009C9.23 10.5009 8.97 10.4509 8.72 10.3409C8.02 10.0309 7.57 9.34086 7.57 8.58086V2.56086C7.57 2.27086 7.31 2.02086 7.02 2.06086C4.22 2.41086 3 4.30086 3 7.00086V17.0009C3 20.0009 4.5 22.0009 8 22.0009H16C19.5 22.0009 21 20.0009 21 17.0009V7.00086C21 4.30086 19.78 2.41086 16.98 2.06086ZM17.5 18.7509H9C8.59 18.7509 8.25 18.4109 8.25 18.0009C8.25 17.5909 8.59 17.2509 9 17.2509H17.5C17.91 17.2509 18.25 17.5909 18.25 18.0009C18.25 18.4109 17.91 18.7509 17.5 18.7509ZM17.5 14.7509H13.25C12.84 14.7509 12.5 14.4109 12.5 14.0009C12.5 13.5909 12.84 13.2509 13.25 13.2509H17.5C17.91 13.2509 18.25 13.5909 18.25 14.0009C18.25 14.4109 17.91 14.7509 17.5 14.7509Z" fill="white" style="fill: var(--fillg);"/>
                    </g>
                    <defs>
                    <clipPath id="clip0_4418_8478">
                    <rect width="24" height="24" fill="white"/>
                    </clipPath>
                    </defs>
                </svg>
                <span>Présentation</span>
            </a>
          </li>

          <li>
            <a href="#">
                <svg class="hidden dark:block" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#fff">
                    <g clip-path="url(#clip0_655_9407)">
                    <path d="M17.5599 12C17.5599 12.4059 17.5118 12.7981 17.4328 13.1766C17.4042 13.3137 17.4687 13.4544 17.594 13.5171L21.0965 15.2683C21.2577 15.3489 21.4534 15.2703 21.5084 15.0987C21.8215 14.1226 21.9999 13.0897 21.9999 12V11.94C21.9999 11.7743 21.8656 11.64 21.6999 11.64H17.8599C17.6942 11.64 17.5599 11.7743 17.5599 11.94V12Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M6.445 12.005C6.4418 12.0018 6.44 11.9975 6.44 11.9929V11.8946C6.44 11.754 6.32603 11.64 6.18544 11.64H2.3C2.13431 11.64 2 11.7743 2 11.94V12C2 13.3901 2.28816 14.704 2.79426 15.9025C2.86394 16.0676 3.06421 16.1286 3.21737 16.0356L6.56313 14.0055C6.68386 13.9323 6.73637 13.785 6.69567 13.6498C6.54128 13.1367 6.45075 12.5882 6.45 12.0171C6.45 12.0126 6.4482 12.0082 6.445 12.005Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M6.14 7.19H2.3C2.13431 7.19 2 7.32432 2 7.49V9.83C2 9.99569 2.13431 10.13 2.3 10.13H6.14C6.30569 10.13 6.44 9.99569 6.44 9.83V7.49C6.44 7.32432 6.30569 7.19 6.14 7.19Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M21.6996 7.19H17.8596C17.6939 7.19 17.5596 7.32432 17.5596 7.49V9.83C17.5596 9.99569 17.6939 10.13 17.8596 10.13H21.6996C21.8653 10.13 21.9996 9.99569 21.9996 9.83V7.49C21.9996 7.32432 21.8653 7.19 21.6996 7.19Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M21.6996 2H17.8596C17.6939 2 17.5596 2.13431 17.5596 2.3V5.39C17.5596 5.55569 17.6939 5.69 17.8596 5.69H21.6996C21.8653 5.69 21.9996 5.55569 21.9996 5.39V2.3C21.9996 2.13431 21.8653 2 21.6996 2Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M17.0658 14.928C16.9228 14.8563 16.7494 14.9101 16.6629 15.0447C16.4235 15.4176 16.1367 15.7556 15.8129 16.0589C15.7003 16.1643 15.6775 16.3364 15.7655 16.463L17.9696 19.6303C18.0679 19.7714 18.2647 19.8016 18.397 19.6919C19.2888 18.9525 20.0474 18.0648 20.6299 17.0595C20.7147 16.9132 20.6556 16.7274 20.5045 16.6517L17.0658 14.928Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M10.9295 21.9321C11.1026 21.9501 11.2502 21.8121 11.2502 21.6382V17.7654C11.2502 17.6139 11.1369 17.487 10.9876 17.4611C10.6124 17.3959 10.2512 17.2917 9.90727 17.1546C9.75678 17.0946 9.58272 17.1552 9.51042 17.3001L7.7969 20.7358C7.72185 20.8863 7.78495 21.0695 7.939 21.1369C8.86964 21.5442 9.87417 21.8227 10.9295 21.9321Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M8.26627 16.4393C8.32749 16.3162 8.29664 16.1679 8.19614 16.0742C7.98618 15.8784 7.79248 15.6669 7.61749 15.4399C7.52017 15.3137 7.3437 15.2748 7.20748 15.3577L3.91769 17.3575C3.7719 17.4461 3.72893 17.6386 3.82782 17.7776C4.4526 18.656 5.21273 19.4339 6.07952 20.071C6.22615 20.1788 6.43266 20.1247 6.51372 19.9618L8.26627 16.4393Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M14.4934 17.2562C14.4104 17.1369 14.2542 17.0951 14.1194 17.1495C13.7651 17.2927 13.3956 17.3979 13.0075 17.4686C12.8604 17.4953 12.75 17.6212 12.75 17.7708V21.6473C12.75 21.8216 12.8981 21.9596 13.0714 21.9416C14.3449 21.8093 15.5423 21.4475 16.6203 20.887C16.7771 20.8055 16.8228 20.6046 16.7218 20.4594L14.4934 17.2562Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M6.14 2H2.3C2.13431 2 2 2.13431 2 2.3V5.39C2 5.55569 2.13431 5.69 2.3 5.69H6.14C6.30569 5.69 6.44 5.55569 6.44 5.39V2.3C6.44 2.13431 6.30569 2 6.14 2Z" fill="white" style="fill: var(--fillg);"/>
                    </g>
                    <defs>
                    <clipPath id="clip0_655_9407">
                    <rect width="24" height="24" fill="white"/>
                    </clipPath>
                    </defs>
                </svg>
                <svg class="block dark:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#000000">
                    <g clip-path="url(#clip0_655_9407)">
                    <path d="M17.5599 12C17.5599 12.4059 17.5118 12.7981 17.4328 13.1766C17.4042 13.3137 17.4687 13.4544 17.594 13.5171L21.0965 15.2683C21.2577 15.3489 21.4534 15.2703 21.5084 15.0987C21.8215 14.1226 21.9999 13.0897 21.9999 12V11.94C21.9999 11.7743 21.8656 11.64 21.6999 11.64H17.8599C17.6942 11.64 17.5599 11.7743 17.5599 11.94V12Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M6.445 12.005C6.4418 12.0018 6.44 11.9975 6.44 11.9929V11.8946C6.44 11.754 6.32603 11.64 6.18544 11.64H2.3C2.13431 11.64 2 11.7743 2 11.94V12C2 13.3901 2.28816 14.704 2.79426 15.9025C2.86394 16.0676 3.06421 16.1286 3.21737 16.0356L6.56313 14.0055C6.68386 13.9323 6.73637 13.785 6.69567 13.6498C6.54128 13.1367 6.45075 12.5882 6.45 12.0171C6.45 12.0126 6.4482 12.0082 6.445 12.005Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M6.14 7.19H2.3C2.13431 7.19 2 7.32432 2 7.49V9.83C2 9.99569 2.13431 10.13 2.3 10.13H6.14C6.30569 10.13 6.44 9.99569 6.44 9.83V7.49C6.44 7.32432 6.30569 7.19 6.14 7.19Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M21.6996 7.19H17.8596C17.6939 7.19 17.5596 7.32432 17.5596 7.49V9.83C17.5596 9.99569 17.6939 10.13 17.8596 10.13H21.6996C21.8653 10.13 21.9996 9.99569 21.9996 9.83V7.49C21.9996 7.32432 21.8653 7.19 21.6996 7.19Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M21.6996 2H17.8596C17.6939 2 17.5596 2.13431 17.5596 2.3V5.39C17.5596 5.55569 17.6939 5.69 17.8596 5.69H21.6996C21.8653 5.69 21.9996 5.55569 21.9996 5.39V2.3C21.9996 2.13431 21.8653 2 21.6996 2Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M17.0658 14.928C16.9228 14.8563 16.7494 14.9101 16.6629 15.0447C16.4235 15.4176 16.1367 15.7556 15.8129 16.0589C15.7003 16.1643 15.6775 16.3364 15.7655 16.463L17.9696 19.6303C18.0679 19.7714 18.2647 19.8016 18.397 19.6919C19.2888 18.9525 20.0474 18.0648 20.6299 17.0595C20.7147 16.9132 20.6556 16.7274 20.5045 16.6517L17.0658 14.928Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M10.9295 21.9321C11.1026 21.9501 11.2502 21.8121 11.2502 21.6382V17.7654C11.2502 17.6139 11.1369 17.487 10.9876 17.4611C10.6124 17.3959 10.2512 17.2917 9.90727 17.1546C9.75678 17.0946 9.58272 17.1552 9.51042 17.3001L7.7969 20.7358C7.72185 20.8863 7.78495 21.0695 7.939 21.1369C8.86964 21.5442 9.87417 21.8227 10.9295 21.9321Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M8.26627 16.4393C8.32749 16.3162 8.29664 16.1679 8.19614 16.0742C7.98618 15.8784 7.79248 15.6669 7.61749 15.4399C7.52017 15.3137 7.3437 15.2748 7.20748 15.3577L3.91769 17.3575C3.7719 17.4461 3.72893 17.6386 3.82782 17.7776C4.4526 18.656 5.21273 19.4339 6.07952 20.071C6.22615 20.1788 6.43266 20.1247 6.51372 19.9618L8.26627 16.4393Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M14.4934 17.2562C14.4104 17.1369 14.2542 17.0951 14.1194 17.1495C13.7651 17.2927 13.3956 17.3979 13.0075 17.4686C12.8604 17.4953 12.75 17.6212 12.75 17.7708V21.6473C12.75 21.8216 12.8981 21.9596 13.0714 21.9416C14.3449 21.8093 15.5423 21.4475 16.6203 20.887C16.7771 20.8055 16.8228 20.6046 16.7218 20.4594L14.4934 17.2562Z" fill="white" style="fill: var(--fillg);"/>
                    <path d="M6.14 2H2.3C2.13431 2 2 2.13431 2 2.3V5.39C2 5.55569 2.13431 5.69 2.3 5.69H6.14C6.30569 5.69 6.44 5.55569 6.44 5.39V2.3C6.44 2.13431 6.30569 2 6.14 2Z" fill="white" style="fill: var(--fillg);"/>
                    </g>
                    <defs>
                    <clipPath id="clip0_655_9407">
                    <rect width="24" height="24" fill="white"/>
                    </clipPath>
                    </defs>
                </svg>
                <span>Montures</span>
            </a>
          </li>
        </ul>
      </div>
    </section>
  </nav>
</aside>
